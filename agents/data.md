...
- AniList memory map + IDB `cache` store (24h TTL): keys `anilist:trending:`,
  ...
  `:anime:<id>`, `:bymal:`, `:search:`, `:spine:`, `:batch:`,
  `:seriesgroup:`, `anilist:viewer`, `anilist:list:<viewerId>`,
  `anilist:media:<id>` (shared record).
...
HashRouter: `#/`, `#/browse`, `#/search?q=`, `#/anime/anilist-<id>`,
`#/watch/<id>/<ep>` (display numbers, season-aware),
`#/list`, `#/settings`, `#/manga`, `#/profile`. Season selection navigates
to the canonical `anilist-<id>` entry (URL = selected season identity).
