...
#### AniList Tracking implementation (Authorization Code, client 50024)

**Authentication (Authorization Code Grant via server):** `response_type=code` + random `state` via `https://anilist.co/api/v2/oauth/authorize?client_id=50024&response_type=code&redirect_uri={origin+BASE_URL}&state={random}`. Implicit grant is rejected by this client (`400 unsupported_grant_type`, proven live twice — do not revert). Callback `?code=&state=` is handled in `AniListContext` effect: state validated against `aeri:anilist:oauth_state`, code exchanged via `POST /api/anilist/token` (Worker injects `ANILIST_CLIENT_SECRET`, or preferred `auth-proxy` on non-Cloudflare hosting since AniList 403-blocks all Cloudflare Worker IPs — see D041; endpoint order `customAuthApiUrl` → `VITE_AUTH_API_URL` → worker/same-origin). Browser never calls `https://anilist.co/api/v2/oauth/token` and never holds the secret. Token stored via `src/storage/anilist.ts` (`aeri:anilist:token`, `aeri:anilist:token_expiry`). MAL codes are ignored by state-match guard (D042). Manual paste fallback (`parseManualToken`) supports raw tokens or code URLs.
...
**Authentication (PKCE `plain` via Worker proxy):** `GET https://myanimelist.net/v1/oauth2/authorize?response_type=code&client_id={VITE_MAL_CLIENT_ID}&code_challenge=<verifier>&code_challenge_method=plain&state=<random>` via `src/services/mal/auth.ts:buildMalAuthorizeUrl`. MAL supports ONLY `plain` (S256 proven broken live — D051). `code_verifier` random 96 chars (43-128), `state` 32 chars, `code_challenge === verifier`. Redirect URI `window.location.origin + import.meta.env.BASE_URL` (`https://fastdemo.github.io/aeri/`). Code is parsed from `window.location.search` (`?code=&state=`) in `src/contexts/MALContext.tsx` effect and `src/services/mal/auth.ts:handleMalOAuthCallback` (state validated, verifier retrieved from storage). Exchange `POST /api/mal/token` on the Worker (preferred; Worker injects `MAL_CLIENT_SECRET` server-side, then proxies `https://myanimelist.net/v1/oauth2/token`) with direct `POST https://myanimelist.net/v1/oauth2/token` fallback (`client_id, grant_type=authorization_code, code, code_verifier`, `Content-Type: application/x-www-form-urlencoded`, `Accept: application/json`) → `{ access_token, refresh_token, expires_in }`. Refresh via `grant_type=refresh_token` when expiry within 1 min (`isMalTokenExpired` buffer), same Worker endpoint (secret injected for both grants). Tokens stored via `src/storage/mal.ts` (`aeri:mal:access_token`, `aeri:mal:refresh_token`, `aeri:mal:token_expiry`, plus `aeri:mal:oauth_state`/`code_verifier` transient). No `client_secret` in the browser, no hard-coded secret — `VITE_MAL_CLIENT_ID` env (fallback empty, manual paste path). Manual paste fallback `parseMalManualToken` supports raw token or URL with `code`.
...
**My List integration:** `src/contexts/MALContext.tsx` mirrors `AniListContext` (`isAuthenticated, user, animeList, loadingUser/loadingList, error, authExpired`, `login/logout/setManualToken/refresh`, optimistic `updateProgress/updateStatus/updateRating` with `malId` resolution from `anime.identity` or the tracker's list). `src/contexts/TrackingContext.tsx` exposes a single active tracker (`trackingProvider`, switchable via `TrackerPicker` in the sign-in modal and Settings; auto AniList-first): reads (`combinedList`) and all mutations go to that account only — no merging, no fan-out. When the tracker is MAL, `src/services/mal/enrichment.ts:enrichMalEntriesWithAnilist` upgrades entries to AniList display metadata via `Media(idMal:)` (cached `anilist:bymal:*`, progressive batches; tracking truth stays MAL). UI (`Home` Continue Watching, `MyList`, `DetailModal`, `EpisodeList`, `Watch`) uses `useTracking`, not direct provider.
...
- **Browse usage:** `src/pages/Browse.tsx` uses `useBrowse` with category tabs `Popular (POPULARITY_DESC)`, `Trending (TRENDING_DESC)`, `Airing (RELEASING)`, `Upcoming (NOT_YET_RELEASED)`, `Finished (FINISHED+END_DATE_DESC)` and filters `genre`, `seasonYear`, `season` (WINTER/SPRING/SUMMER/FALL), `format` (TV/MOVIE/OVA/SPECIAL) as server-side AniList filters. `perPage = columns × 5` via `useGridColumns()` (10/15/20/25/30 for 2/3/4/5/6 cols) so every fetch renders complete rows at any viewport; pagination via `hasNextPage` + `Load more` (appends full rows). Loading shows `perPage` pulse slots, error shows retry, empty shows “No titles match your filters.” No mock.
- **Search usage:** `src/pages/Search.tsx` uses `useAnimeSearch(liveQuery, 12)` (debounced, URL-synced). Navbar `SearchSuggestions` opens the shared `DetailModal` popup (selection lifted to `Navbar`, no `/anime/:id` navigation); option buttons carry `type="button"` so picks never submit the wrapping search form. `/anime/:id` route remains for deep links.
...
- Authorized sources only, no DRM bypass, no secrets in `VITE_*` (client IDs are public; `ANILIST_CLIENT_SECRET` lives only in Worker secrets / auth-proxy env, never in bundles).
...
- Early `?code=` handling: AniList and MAL callbacks are both search params and survive HashRouter; each context claims only codes whose `state` matches its own stored state (D042). No hash-token parsing remains.
