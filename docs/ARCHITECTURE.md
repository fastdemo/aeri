...
- **TrackingProvider**: two implementations behind same interface:
  ...
  - `src/providers/mal/provider.ts` (`MALProvider`, Phase 5): user-specific, PKCE `plain` (challenge === verifier; S256 proven broken live, D051), `GET /users/@me`, `GET /users/@me/animelist` paginated + `GET /anime/{id}` + `PUT /anime/{id}/my_list_status`. Token exchange via Worker `/api/mal/token` (injects `MAL_CLIENT_SECRET` server-side). Context `MALContext` mirrors `AniListContext` with `ensureFreshToken` refresh and `clearMalMemoryCache`. Both through `src/services/mal/*` + `src/lib/malConfig.ts` + `src/storage/mal.ts`.
  - `src/contexts/TrackingContext.tsx`: single active tracker (`trackingProvider: 'anilist' | 'mal'`, auto AniList-first, switchable in sign-in modal + Settings). Reads (`combinedList`) and all mutations go to that account only — no cross-provider merge, no fan-out. MAL tracker lists are upgraded to AniList display metadata via `Media(idMal:)` enrichment (`src/services/mal/enrichment.ts`, cached, progressive). UI consumes `useTracking`, never direct providers. Discovery/metadata is AniList-only in all cases.
