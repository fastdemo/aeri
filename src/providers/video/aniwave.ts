...
export class AniWaveProvider implements VideoProvider {
  ...
  async getEpisodes(anime: Anime, signal?: AbortSignal): Promise<VideoEpisode[]> {
    ...
    return cachedFetch(`video:aniwave:episodes:${anime.identity.anilistId}`, async () => {
      try {
        const res = await fetchWithTimeout(`${base}/api/episodes/${anime.identity.anilistId}?provider=aniwave&title=${encodeURIComponent(titleHint)}${typeof anime.episodes === 'number' && anime.episodes > 0 ? `&episodes=${anime.episodes}` : ''}${anime.format ? `&format=${encodeURIComponent(anime.format)}` : ''}${typeof anime.year === 'number' && anime.year > 1900 ? `&year=${anime.year}` : ''}`, {}, 3500, signal)
        if (!res.ok) return []
        const j: any = await res.json().catch(() => null)
  ...
  async getSources(episode: VideoEpisode, options?: SourceOptions): Promise<VideoSourceEnhanced[]> {
    ...
    try {
      const url = `${base}/api/sources/aniwave-${anilistId}-${episode.number}?language=${lang}&provider=aniwave${titleHint ? `&title=${encodeURIComponent(titleHint)}` : ''}${hintQuery(options)}`
